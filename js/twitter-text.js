...
(function() {
  ...
  */
   ...
   * (the UTF-16 code unit matches the Unicode code point
   * for code points representable in a single UTF-16 code unit)
   * 
   */
  // 
  var HIGH_CODE_POINT = 65536;
  ...
  twttr.txt.getTweetLength = function(text, options) {
    if (!options) {
      options = {
        ...
        short_url_length: config.transformedURLLength,
        short_url_length_https: config.transformedURLLength
      };
    }
    ...
    while (charOffset < text.length) {
      var codePoint = text.codePointAt(charOffset)
      var temp = weightForCodePoint(codePoint);
      weightedLength += temp;
      charOffset += codePoint < HIGH_CODE_POINT ? 1 : 2;
    }
    var urlsWithIndices = twttr.txt.extractUrlsWithIndices(text);
    twttr.txt.modifyIndicesFromUTF16ToUnicode(text, urlsWithIndices);
    
    var textLength = weightedLength / config.scale;
    for (var i = 0; i < urlsWithIndices.length; i++) {
